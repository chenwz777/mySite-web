...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        init: function () {
        	if(typeof  this.options.areaId != 'undefined'){
           		 this.options.province = this.options.areaId.substring(0,2);
           		 this.options.city = this.options.areaId.substring(0,4);
           		 this.options.district = this.options.areaId;
            }
            ...
            if(typeof this.options.province != 'undefined' && this.options.province != ''){
        		this.selectCallBack();
            }
        },
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span text_over" style="' +
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    '<span id="closeBtn" class="r_btn">关闭</span>' +
                    '<span id="resetBtn" class="r_btn">重置</span>' +
                    '<a class="province-first active" data-count="province">省份</a>' +
        ...
        bind: function () {
            ...
            }).on('click', '.city-select-tab #closeBtn', function () {
            	$this.close();
            }).on('click', '.city-select-tab #resetBtn', function () {
        ...
        selectCallBack: function () {
        	...
        	try{
			        ...
			        if(val.lastIndexOf('/') > -1){
			        	newVal = val.substr(val.lastIndexOf('/') + 1);
			            newText = text.substr(text.lastIndexOf('/') + 1);
			        }
			        
			        $('#s_'+ id).val(newVal);				        
			        $('#s_'+ id).attr('dataText', text.replace(/\//g,''));
			        
			        if(typeof(selectAreaed) == "function" && selectAreaed){
                		selectAreaed(id, newVal, newText, val, text);
                	}
                }catch(e){}
        }
    };
